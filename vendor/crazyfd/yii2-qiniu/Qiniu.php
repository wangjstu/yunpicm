...
class Qiniu
{
    ...
    public function accessToken($url, $body = false)
    {
        ...
        $digest = hash_hmac('sha1', $url, $this->secretKey, true);
...
//         $url = "http://{$this->domain}/{$key}";
    ...
    //私有资源下载
    public function getAuthLink($key = '', $data = array())
    {
        $url = 'http://'.rtrim($this->domain,'/')."/{$key}".'?';
        if (isset($data['imageView2'])) { //裁剪图片  :http://developer.qiniu.com/code/v6/api/kodo-api/image/imageview2.html
            $url.= 'imageView2/';
            $url.= isset($data['imageView2']['mode']) ? $data['imageView2']['mode'] : '1';
            $url.= isset($data['imageView2']['w']) ? '/w/'.$data['imageView2']['w'] :'/w/200';
            $url.= isset($data['imageView2']['h']) ? '/h/'.$data['imageView2']['h'] :'/h/200';
            $url.= isset($data['imageView2']['format']) ? '/format/'.$data['imageView2']['format'] :'';
            $url.= isset($data['imageView2']['interlace']) ? '/interlace/'.$data['imageView2']['interlace'] :'';
            $url.= isset($data['imageView2']['q']) ? '/q/'.$data['imageView2']['q'] :'';
            $url.= isset($data['imageView2']['ignore-error']) ? '/q/'.$data['imageView2']['ignore-error'] :'';
        }
        $url.= isset($data['attname']) ? '&attname='.$data['attname'] : '&attname=';
        $url.= isset($data['deadline']) ? '&e='.$data['deadline'] : '&e='.(time()+3600);
        $url.= '&token=' . $this->accessToken($url);
        return $url;
    }
